package co.youngleader.rhino.adminweb.rest.resource.podcast;
...
import co.youngleader.rhino.adminweb.rest.service.PodcastService;
...
public class PodcastsResource {
	...
	public Response createPodcastFromApplicationFormURLencoded(@FormParam("title") String title,
		...
		return Response.status(Response.Status.CREATED)// 201
				.entity("A new podcast/resource has been created at /admin-webapp-rhino/podcasts/"
						+ createPodcastid)
				.header("Location",
						"http://localhost:8080/admin-webapp-rhino/podcasts/" + String.valueOf(createPodcastid))
				.build();
	}
	 ...
	 */
	@GET
	// @Compress //can be used only if you want to SELECTIVELY enable
	// compression at the method level. By using the EncodingFilter everything
	// is compressed now.
	...
	public Response putPodcastById(@PathParam("id") Long id, Podcast podcast) throws AppException {
		...
		if (podcastById == null) {
			...
			return Response.status(Response.Status.OK)
					...
					.header("Location", "http://localhost:8888/demo-rest-jersey-spring/podcasts/" + String.valueOf(id))
					.build();
		}
	}
